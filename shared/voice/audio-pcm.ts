/**
 * Pure audio-format helpers for a platform-ai voice session (game-agnostic).
 *
 * Side-effect-free: no DOM / Web Audio / WebSocket access at module top level, so
 * these are safe to import from any React hook (the in-game `useVoiceSession`, the
 * lobby `useLobbyVoiceSession`) and from vitest unit tests. The hooks own all Web
 * Audio side effects; this module only converts byte/sample representations.
 *
 * Two directions are needed by every voice session:
 ...
 *
 * Linear interpolation is sufficient for speech (the accepted trade-off): output
 * sample `i` lands at input position `i * fromRate / toRate` and is the linear
 * blend of its two neighbouring input samples. When the rates already match (or
 * the input is empty) the input is returned unchanged, so the common 16 kHz path
 * stays allocation-free. Downsampling applies no anti-alias pre-filter —
 * acceptable for the narrow-band speech the STT consumes.
 */
