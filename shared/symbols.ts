 ...
 * strings: `packages/manual/build.ts` injects each referenced symbol's
 * description at build time into the canonical AI payload — both the YAML
 * embedded in the rendered manual HTML (`dist/<slug>/index.html`) and the
 * dist raw YAML the AI fetches via `?format=yaml`. The source YAML never
 * carries a `symbols:` block; descriptions are owned here and injected.
...
export const SYMBOLS: readonly Symbol[] = [
  ...
  },
  {
    ...
    name: 'Psi',
    description:
      "U 形碗:两臂从两侧向上、底部弯合成一只碗,正中一根长竖线穿过碗并向下延伸超出碗底。与 trident 的关键区别——这里是单竖线穿过一只连续的 U 形碗(两臂在底部弯合),而非三股彼此分离、靠横梁相连的直尖齿;容易被误描述为'三叉戟'",
  ...
  },
  {
    ...
    name: 'Trident',
    description:
      "三叉戟:顶部三股分离的直尖齿并排朝上,齿根由一条水平横梁连成一排,横梁正中再伸出一根竖直长柄一路向下,整体像渔叉/三股叉。与 psi 的关键区别——三齿是彼此分离的直竖线、底下靠一条横梁相连,而 psi 是单竖线穿过一只 U 形碗(两臂在底部弯合);容易被误描述为'叉子'",
    path: 'M28 14 L28 40 M50 12 L50 40 M72 14 L72 40 M24 40 L76 40 M50 40 L50 90',
  },
  {
    id: 'crescent',
    name: 'Crescent',
