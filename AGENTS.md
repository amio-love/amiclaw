...
- The leaderboard shows time, attempt number, and the AI tool used when provided.
...
- Puzzle answer calculation is client-side in the MVP. Do not move it to the server without flagging the security tradeoff.
- Never add a light mode. The product is dark-only by design.
- Do not add JavaScript animation libraries. CSS-only animation is a hard constraint.
