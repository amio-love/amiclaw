...
import { checkBotanicalVoiceEligibility } from './voice-eligibility'
...
describe('Botanical voice eligibility preflight', () => {
  it('checks same-origin auth before the existing companion identity', async () => {
    const fetcher = vi
      .fn<typeof fetch>()
      .mockResolvedValueOnce(json({ authenticated: true }))
      .mockResolvedValueOnce(json({ name: '小苗' }))

    await expect(checkBotanicalVoiceEligibility(fetcher)).resolves.toEqual({
      status: 'eligible',
      companionName: '小苗',
    })
  ...
  it('stops before companion lookup for an anonymous player (→ manual mode)', async () => {
