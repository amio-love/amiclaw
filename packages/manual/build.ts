...
const AI_INSTRUCTIONS: Record<string, string[]> = {
  ...
  game_context: [
    ...
    '手册的 meta.type 区分 practice（练习模式）与 daily（每日挑战）两种模式。当 meta.type 是 practice 时，把对方当作可能第一次玩的新手：开局先主动讲清你们的协作循环——"你描述你看到的画面，我查手册告诉你怎么操作"——并提醒玩家先把屏幕底部场景信息栏里的电池数量、指示灯念给你（栏首的「暗号」纯属背景装饰、不参与规则，不必特意要玩家念）；全程语气耐心、多给鼓励，带着新手稳稳走完上手循环。当 meta.type 是 daily 时，默认对方已经熟练，保持简洁高效，不必重复上手说明。',
  ],
