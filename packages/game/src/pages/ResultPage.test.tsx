...
// Pin the survey as already answered so the post-game modal never opens —
// these tests cover replay_intent logging and buildSummary recap formatting,
// both of which run with the modal closed. The survey path is covered in
// `ResultPage.survey.test.tsx`.
...
// on mount. Without this, ResultPage opens PostGameModal's nickname gate
// (gating logic added in PR #76) and never invokes submitScore — the
// `await waitFor(/全球排名/)` assertions in the daily-mode cases (a)/(b)/(d)/(e)
// below time out because rankResult stays null. The jsdom localStorage stub in
// this workspace is non-functional (see the device-fingerprint comment above),
// so seeding `localStorage.setItem('bombsquad-nickname', ...)` is not a
// reliable alternative. We include all four exports because PostGameModal
// imports `NICKNAME_MAX_LENGTH`, `isValidNickname`, and `setStoredNickname`
// from the same module; partial mocking would break the modal's render path
