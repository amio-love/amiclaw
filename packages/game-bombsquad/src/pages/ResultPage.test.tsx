...
// Mock the leaderboard API so the on-mount daily score submission resolves to a
// controllable shape. The replay_intent test runs in practice mode (never
// submits), so the default vi.fn() is enough.
vi.mock('@shared/leaderboard-api', () => ({
  submitScore: vi.fn(),
}))

// Pin the survey as already answered so the post-game modal never opens —
// this test covers replay_intent logging, which runs with the modal closed.
// The survey path is covered in
...
// (gating logic added in PR #76) and never invokes submitScore. The jsdom
// localStorage stub in this workspace is non-functional (see the
// device-fingerprint comment above), so seeding
// `localStorage.setItem('bombsquad-nickname', ...)` is not a reliable
// alternative. We include all four exports because PostGameModal
