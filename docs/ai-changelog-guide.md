...
This repository keeps `CHANGELOG.md` updated continuously. Every change that
lands on `main` should be recorded directly below the `Unreleased` header
before release day.

You will receive:

1. **A raw draft** — generated from commit history, PRs, or rough release notes
...
Auto-generated changelogs describe what developers _did_.
Your job is to describe what users _get_.
...
Every sentence should answer: _"Why does this matter to the person using the product?"_
...
The logger refactor is omitted because it has no user-visible effect.
