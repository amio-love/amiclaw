 ...
 *
 * Runs the game's OWN seeded RNG + module generators against a pinned
 * timestamp `T` and writes e2e/fixtures/answers.json — the mechanical answer
 * for every module of a deterministic daily and practice run. The harness
 * plays a real, full play-through using these answers; it never re-implements
 * a solver. Both modes now seed from Date.now() (utils/session.ts getRunSeed —
 * practice re-randomizes per run under its fixed manual), so BOTH runs are
 * built at the same pinned `T` the harness freezes the clock to. The daily run
 * uses the daily-manual fixture (its network load is route-mocked); the
 * practice run uses the REAL packages/manual/data/practice.yaml, because the
 * app inlines that file at build time (`?raw`) and no route mock can touch it.
 *
...
function main() {
  ...
  const practiceManualPath = resolve(__dirname, '../../packages/manual/data/practice.yaml')
  const practiceManual = yaml.load(readFileSync(practiceManualPath, 'utf8')) as Manual

  // Pick a timestamp T whose daily button puzzle resolves to action:'tap' AND
  // whose practice run generates cleanly — both modes seed from the same
  // pinned clock. Base: 2026-05-22T12:00:00Z, then step forward 1 ms at a time.
  const BASE_T = Date.UTC(2026, 4, 22, 12, 0, 0)
  let chosenT = -1
  let daily: { scene: SceneInfo; answers: ModuleAnswerEntry[] } | null = null
  let practice: { scene: SceneInfo; answers: ModuleAnswerEntry[] } | null = null
  for (let i = 0; i < 200_000; i++) {
    ...
    try {
      ...
      if ((button?.answer as { action?: string })?.action === 'tap') {
        chosenT = candidate
        daily = run
        practice = buildRun(PRACTICE_SEQUENCE, candidate, practiceManual)
        break
      }
    } catch {
  ...
  if (chosenT < 0 || daily === null || practice === null) {
    throw new Error('build-answers: no timestamp produced a tap-button daily + clean practice run')
  }

  const out = {
    // Pinned run-seed for BOTH modes. getRunSeed() returns Date.now(); the
